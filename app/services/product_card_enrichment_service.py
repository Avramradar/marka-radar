# MARKARADAR TARGETED ENRICHMENT V2 — 2026-08-11
...
def build_targeted_enrichment_queries( *, product: Product, brand: Brand, ) -> tuple[str, ...]:
    """ Формирует поисковые запросы для ОБОГАЩЕНИЯ УЖЕ ВЫБРАННОГО SKU. Первый запрос максимально точный: бренд + название + упаковка. Второй — fallback без упаковки, если каталог плохо индексирует вес. Важное отличие от общего каталожного поиска: результаты этих запросов НИКОГДА не создают новый Product. """
...
def _target_package_match( *, product: Product, external_product: Any, ) -> bool | None:
    """ Более строгая проверка упаковки, чем обычный Merge Engine. Для targeted enrichment нельзя брать фото 240 г для карточки 245 г. Допуск только 0.5%, чтобы пережить техническое округление, но не объединять соседние SKU. """
...
def _target_candidate_identity( *, product: Product, brand: Brand, external_product: Any, needed_fields: tuple[str, ...], ) -> tuple[bool, float, str]:
    """ Проверяет, можно ли использовать внешнюю карточку ТОЛЬКО для обогащения выбранного product_id. Здесь намеренно правила строже, чем у общего каталогового импорта: лучше пропустить фото, чем присвоить изображение соседнего SKU. """
...
async def _merge_external_into_target( *, session: AsyncSession, state: ProductCardState, external_product: Any, identity_reason: str, ) -> tuple[str, ...]:
    """ Обогащает только state.product. НИКОГДА не вызывает общий merge_external_product(), поэтому в режиме открытия карточки не может создать соседний Product. """
...
async def _clear_invalid_target_image( *, session: AsyncSession, state: ProductCardState, ) -> ProductCardState:
...
async def _try_targeted_external_product( *, session: AsyncSession, state: ProductCardState, external_product: Any, needed_fields: tuple[str, ...], ) -> tuple[ProductCardState, bool]:
...
async def ensure_product_card_enriched( *, session: AsyncSession, product_id: int, limit_per_provider: int = 8, ) -> ProductCardState:
    """ Targeted enrichment выбранного товара. Главный инвариант: этот процесс может улучшать ТОЛЬКО переданный product_id и не имеет права создавать новые товары из поисковой выдачи провайдера. Порядок: локальная карточка -> проверка полноты -> barcode lookup каждого провайдера -> строгий текстовый поиск -> merge только в target -> повторная проверка -> остановка после полной карточки. """
